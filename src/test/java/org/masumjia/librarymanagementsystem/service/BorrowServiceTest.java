...
public class BorrowServiceTest {
    @Test
    void borrow_ok(){
        BorrowRecordRepository repo = Mockito.mock(BorrowRecordRepository.class);
        BookService bookSvc = Mockito.mock(BookService.class);
        UserService userSvc = Mockito.mock(UserService.class);

        Book book = new Book();
        book.setId(1L); book.setAvailable(true);
        User user = new User();
        user.setId(1L);
